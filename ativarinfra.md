...
- **CONFIGURADO/VALIDADO — Supabase:** CLI vinculada exclusivamente a `bscpfutlmsvbwgtkdudv`, projeto `doctormd`, região `us-east-1`, estado `ACTIVE_HEALTHY`. Há 16 migrations versionadas, de `20260807193000_product_foundation.sql` a `20260908195000_add_takeover_greeting.sql`, cobrindo fundação, identidade/acesso, chat, proteção contra spoofing, red flags/takeover, automações interativas, IA contextual, resumos, captação comercial isolada por organização, catálogo canônico de red flags pós-operatórias, confirmação explícita do paciente e saudação automática no takeover médico.
...
- Alteração estrutural mais recente: migration `20260908195000_add_takeover_greeting.sql`, aplicada ao projeto `bscpfutlmsvbwgtkdudv` em 2026-09-08, que torna atômicos o takeover médico e o envio da saudação padronizada ao paciente.
