...
- **CONFIGURADO/VALIDADO — GitHub:** `origin` oficial, branch `main`, conta com permissão `WRITE`. Em 2026-08-12, os 9 commits que estavam somente no ambiente local foram enviados e `main`/`origin/main` ficaram sincronizados em `3fcb4de`. Continue confirmando o SHA antes de operações mutáveis e nunca use pull/reset/rebase destrutivo para resolver diferenças sem revisão.
- **CONFIGURADO/VALIDADO — Supabase:** CLI vinculada exclusivamente a `bscpfutlmsvbwgtkdudv`, projeto `doctormd`, região `us-east-1`, estado `ACTIVE_HEALTHY`. Há 13 migrations versionadas, de `20260807193000_product_foundation.sql` a `20260819163000_scope_sales_leads_to_organization.sql`, cobrindo fundação, identidade/acesso, chat, proteção contra spoofing, red flags/takeover, automações interativas, IA contextual, resumos e captação comercial isolada por organização.
- **CONFIGURADO/VALIDADO — leads comerciais:** `public.sales_leads` recebe solicitações da landing exclusivamente pela ação server-side com `SUPABASE_SERVICE_ROLE_KEY`; RLS está ativa e todos os privilégios foram removidos de `anon` e `authenticated`. Não armazenar dados clínicos nessa tabela. O formulário possui consentimento, honeypot e limitação de três envios por e-mail/hora.
- **CONFIGURADO/VALIDADO — Vercel:** conta `lucianoterresrosa@gmail.com`, identidade CLI `lucianoterresrosa-6245`, team `luciano-terres-projects`, projeto `apollomd`, URL `https://apollomd.vercel.app`. A produção funcional foi publicada pela CLI a partir de `d9bf3b1`; o GitHub passou a conter esse código e a documentação de infraestrutura em `3fcb4de`. Um deploy automático disparado pelo push pode atualizar a produção para o commit documental sem alterar o runtime.
...
- Data: 2026-08-19 (America/Sao_Paulo)
- GitHub sincronizado em 2026-08-12: `main` e `origin/main` confirmados no SHA completo `3fcb4deb014e9b89cfe2c2f22360533a504ad189` antes desta atualização final do documento.
...
- Alteração estrutural mais recente: migration `20260819163000_scope_sales_leads_to_organization.sql`, que vincula obrigatoriamente cada lead comercial ao tenant responsável, aplicada ao projeto `bscpfutlmsvbwgtkdudv` em 2026-08-19 após dry-run exclusivo.
