...
- **CONFIGURADO/VALIDADO — Supabase:** CLI vinculada exclusivamente a `bscpfutlmsvbwgtkdudv`, projeto `doctormd`, região `us-east-1`, estado `ACTIVE_HEALTHY`. Há 12 migrations versionadas, de `20260807193000_product_foundation.sql` a `20260819150000_sales_leads.sql`, cobrindo fundação, identidade/acesso, chat, proteção contra spoofing, red flags/takeover, automações interativas, IA contextual, resumos e captação comercial.
...
- Alteração estrutural mais recente: migration `20260819150000_sales_leads.sql` aplicada ao projeto `bscpfutlmsvbwgtkdudv` em 2026-08-19 após dry-run exclusivo.
