...
- **CONFIGURADO/VALIDADO — Supabase:** CLI vinculada exclusivamente a `bscpfutlmsvbwgtkdudv`, projeto `doctormd`, região `us-east-1`, estado `ACTIVE_HEALTHY`. Há 19 migrations versionadas, de `20260807193000_product_foundation.sql` a `20260909170000_assign_responsible_doctor_to_patients.sql`, cobrindo fundação, identidade/acesso, chat, proteção contra spoofing, red flags/takeover, automações interativas, IA contextual, resumos, captação comercial isolada por organização, catálogo canônico de red flags pós-operatórias, confirmação explícita do paciente, saudação automática no takeover médico, padronização da marca, acesso integrado dos médicos e atribuição do responsável no cadastro do paciente.
...
- **CONFIGURADO/VALIDADO — médico responsável do paciente:** em 2026-09-09, o cadastro de paciente passou a exigir um médico responsável e novos acompanhamentos passaram a herdar essa atribuição. Administradores e equipe podem trocar o responsável no detalhe do paciente; a operação atualiza de forma transacional os acompanhamentos planejados, pré-operatórios e pós-operatórios e preserva o médico dos acompanhamentos concluídos ou cancelados como histórico. A migration `20260909170000_assign_responsible_doctor_to_patients.sql`, aplicada ao projeto `bscpfutlmsvbwgtkdudv` após dry-run exclusivo, adicionou `patients.responsible_doctor_id`, auditou reatribuições e adotou o responsável atual como escopo principal da RLS do paciente.
...
- Alteração estrutural mais recente: migration `20260909170000_assign_responsible_doctor_to_patients.sql`, aplicada ao projeto `bscpfutlmsvbwgtkdudv` em 2026-09-09 após dry-run exclusivo, que adiciona o responsável ao cadastro do paciente e permite sua troca transacional nos acompanhamentos ativos.
