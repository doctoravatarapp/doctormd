...
- **CONFIGURADO/VALIDADO — Supabase:** CLI vinculada exclusivamente a `bscpfutlmsvbwgtkdudv`, projeto `doctormd`, região `us-east-1`, estado `ACTIVE_HEALTHY`. Há 17 migrations versionadas, de `20260807193000_product_foundation.sql` a `20260908210000_standardize_apollomd_brand.sql`, cobrindo fundação, identidade/acesso, chat, proteção contra spoofing, red flags/takeover, automações interativas, IA contextual, resumos, captação comercial isolada por organização, catálogo canônico de red flags pós-operatórias, confirmação explícita do paciente, saudação automática no takeover médico e padronização da marca.
...
- Alteração estrutural mais recente: migration `20260908210000_standardize_apollomd_brand.sql`, aplicada ao projeto `bscpfutlmsvbwgtkdudv` em 2026-09-08, que padroniza a marca também nos valores persistidos e defaults do Supabase.
