...
const db=createClient(url,key,{auth:{persistSession:false}}); const made=[],madeMessages=[];
...
async function fixture(answer,{invalid=false,human=false}={}){
 const episode=ok(await db.from("care_episodes").select("*,patients!inner(id)").not("started_at","is",null).limit(1).single(),"episode");
 ...
 const ea=await one("episode_automations",db.from("episode_automations").insert({organization_id:episode.organization_id,care_episode_id:episode.id,flow_id:flow.id,flow_version:1,status:"waiting_response",current_step_id:q.id}),"assignment");
...
try{await fixture("9");await fixture("4");await fixture("abc",{invalid:true});await fixture("9",{human:true});console.log("interactive automation E2E: OK");}
finally{for(const id of made.reverse())await db.from("automation_flows").delete().eq("id",id);if(madeMessages.length)await db.from("messages").delete().in("id",madeMessages);}
