...
const groups: {label:string;items:NavItem[]}[] = [
  ...
  { label: "Gestão", items: [{ href: "/admin/leads", label: "Leads", icon: "↗", adminOnly:true }, { href: "/admin/doctors", label: "Médicos", icon: "+" }, { href: "/admin/automations", label: "Automações", icon: "◇" }, { href: "/admin/team", label: "Equipe", icon: "◎" }] },
  { label: "Sistema", items: [{ href: "/admin/settings", label: "Configurações", icon: "⚙" }] },
];

export function Sidebar({ organizationName, email, userName, role, canViewLeads, alertCount = 0, open, onClose }: { organizationName: string; email: string | null; userName: string; role: string;canViewLeads:boolean; alertCount?: number; open: boolean; onClose: () => void }) {
