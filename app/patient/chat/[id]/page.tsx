...
export default async function PatientChatPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: doctor }, { data: messages }, { data: assignment }, {data:episodes}, {data:pendingRedFlag}] = await Promise.all([supabase.from("doctors").select("display_name").eq("id", episode.doctor_id).single(), supabase.from("messages").select("id, sender_type, content, created_at").eq("conversation_id", conversation.id).order("created_at"),supabase.from("episode_automations").select("id,current_step_id,status").eq("care_episode_id",conversation.care_episode_id).eq("status","waiting_response").maybeSingle(),supabase.from("care_episodes").select("id,procedure_name,status").eq("patient_id",patient.id).in("status",["planned","preoperative","postoperative"]),supabase.from("red_flag_confirmations").select("id").eq("conversation_id",conversation.id).eq("status","pending").maybeSingle()]);
  ...
  return <PatientShell chat patientName={patient.preferred_name||patient.full_name} currentTitle={episode.procedure_name} currentSubtitle={`${doctor?.display_name || "Equipe de cuidado"}`} conversations={items}><main className="patient-chat-shell"><PatientChat conversationId={conversation.id} initialMessages={messages ?? []} initialMode={conversation.mode} initialQuestion={question} initialRedFlagConfirmation={Boolean(pendingRedFlag)} /></main></PatientShell>;
}
