...
export default async function EpisodePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ assigned?: string; error?: string; summary?: string }> }) {
  ...
  return <main className="admin-content">
    ...
    <section className="detail-grid">
      <article className="panel"><div className="episode-conversation-header"><div><h2>Conversa</h2><p className="muted-copy">Mensagens e fontes originais deste acompanhamento.</p></div>{conversation ? <Link className="primary-action-link" href={`/admin/conversations/${conversation.id}`}>Abrir conversa <span aria-hidden="true">→</span></Link> : null}</div>{conversation ? null : <p>Nenhuma conversa vinculada a este acompanhamento.</p>}{messages?.slice(0, 8).map((row) => <div className="compact-row" id={`message-${row.id}`} key={row.id}><div><strong>{row.sender_type}</strong><small>{row.content}</small></div><span>{format(row.created_at)}</span></div>)}</article>
