...
export async function createEpisode(formData: FormData) {
  ...
  if (!context.organization || !can(context.role, "episodes:create")) redirect(`/admin/patients/${patientId}/episodes/new?error=access`);
  ...
  if (!patientId || !doctorId || procedureName.length < 2) redirect(`/admin/patients/${patientId}/episodes/new?error=validation`);
  ...
  if (error || !episodeId) redirect(`/admin/patients/${patientId}/episodes/new?error=episode`);
