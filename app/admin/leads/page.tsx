...
export default async function LeadsPage({searchParams}:{searchParams:Promise<{q?:string;status?:string;updated?:string}>}){
  ...
  const admin=createAdminClient(),q=(params.q??"").trim().replace(/[,%_()]/g," ").slice(0,80),status=validStatuses.has(params.status??"")?params.status:"",organizationId=context.organization.id;
  let query=admin.from("sales_leads").select("id,full_name,email,phone,organization_name,team_size,primary_goal,status,created_at").eq("organization_id",organizationId).order("created_at",{ascending:false}).limit(100);
  ...
  const [{data:leads},{data:allStatuses}]=await Promise.all([query,admin.from("sales_leads").select("status").eq("organization_id",organizationId)]);
