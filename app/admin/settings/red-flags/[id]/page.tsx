...
export default async function RedFlagDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  return <main className="admin-content crud-detail"><PageHeader eyebrow="RED FLAG" title={rule.name} description={canonical ? "Regra do catálogo base. As alterações serão aplicadas a esta organização." : "Regra adicional definida pela equipe."} />
    ...
    <section className="panel"><RedFlagRuleForm rule={{ id:rule.id,category:rule.category||"",signal:rule.signal,recommendedAction:rule.recommended_action||"",priority:rule.priority||"contact_surgeon",similarExpressions:configuration.matcher?.phrases?.filter(item=>item!==rule.signal).join("\n")||"" }} /></section>
