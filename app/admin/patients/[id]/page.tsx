...
import { assignPatientDoctor } from "../actions";
...
export default async function PatientDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; saved?: string }> }) {
  ...
  const { data: patient } = await supabase.from("patients").select("id, full_name, preferred_name, email, phone, birth_date, status, auth_user_id, responsible_doctor_id").eq("organization_id", context.organization.id).eq("id", id).maybeSingle();
  if (!patient) notFound();
  const [{ data: episodes }, { data: conversations }, { data: doctors }] = await Promise.all([
    ...
    supabase.from("doctors").select("id, display_name, specialty, status").eq("organization_id", context.organization.id).order("display_name"),
  ]);
  ...
  const responsibleDoctor = doctors?.find((doctor) => doctor.id === patient.responsible_doctor_id);
  const canReassign = ["platform_admin", "organization_admin", "staff"].includes(context.role);
  ...
  return <main className="admin-content patient-detail-page"><PageHeader eyebrow="PACIENTE" title={patient.preferred_name || patient.full_name} description={patient.full_name} action={<div className="header-actions"><Link className="secondary-link" href={`/admin/patients/${patient.id}/edit`}>Editar dados</Link><Link className="primary-link" href={`/admin/patients/${patient.id}/episodes/new`}>Novo acompanhamento</Link></div>} />
    {query.saved ? <p className="success-message">{query.saved === "doctor" ? "Médico responsável atualizado nos acompanhamentos ativos." : "Paciente atualizado."}</p> : null}{query.error ? <p className="form-error">Não foi possível concluir. Revise os dados.</p> : null}
    ...
    <section className="patient-detail-layout"><div className="patient-detail-main"><article className="panel"><div className="panel-title"><h2>Acompanhamentos</h2><Link href={`/admin/patients/${patient.id}/episodes/new`}>Adicionar</Link></div>{episodes?.length ? episodes.map((episode) => <Link className="patient-episode-row" href={`/admin/episodes/${episode.id}`} key={episode.id}><div><strong>{episode.procedure_name}</strong><small>{doctorNames.get(episode.doctor_id) || "Médico não encontrado"}{episode.procedure_date ? ` · ${new Intl.DateTimeFormat("pt-BR").format(new Date(`${episode.procedure_date}T12:00:00`))}` : ""}</small></div><span className="status-badge">{episode.status}</span><b>→</b></Link>) : <EmptyState title="Nenhum acompanhamento" description="Crie o primeiro acompanhamento para iniciar a jornada deste paciente." />}</article>
      ...
      <aside><section className="panel patient-profile-card"><div className="panel-title"><h2>Dados do paciente</h2><Link href={`/admin/patients/${patient.id}/edit`}>Editar</Link></div><dl><div><dt>E-mail</dt><dd>{patient.email || "Não informado"}</dd></div><div><dt>Telefone</dt><dd>{patient.phone || "Não informado"}</dd></div><div><dt>Nascimento</dt><dd>{birthDate}</dd></div><div><dt>Nome completo</dt><dd>{patient.full_name}</dd></div></dl></section><section className="panel patient-profile-card"><h2>Médico responsável</h2>{canReassign ? <form action={assignPatientDoctor} className="settings-form"><input type="hidden" name="patient_id" value={patient.id} /><label>Responsável atual<select name="responsible_doctor_id" defaultValue={patient.responsible_doctor_id || ""} required><option value="" disabled>Selecione um médico</option>{doctors?.filter((doctor) => doctor.status === "active" || doctor.id === patient.responsible_doctor_id).map((doctor) => <option key={doctor.id} value={doctor.id}>{doctor.display_name}{doctor.specialty ? ` · ${doctor.specialty}` : ""}{doctor.status === "inactive" ? " · Inativo" : ""}</option>)}</select></label><p className="muted-copy">A troca transfere os acompanhamentos ativos e suas conversas. Acompanhamentos concluídos ou cancelados preservam o histórico original.</p><button>Alterar médico responsável</button></form> : <p>{responsibleDoctor?.display_name || "Não definido"}</p>}</section></aside></section>
  </main>;
}
