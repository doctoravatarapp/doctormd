...
export async function POST(request: Request) {
  ...
  if (!conversation?.care_episode_id || conversation.status !== "open" || conversation.mode !== "ai") return new Response("Conversa não autorizada.", { status: 403 });
  ...
  const admin = createAdminClient(); const staleAt = new Date(Date.now() - AI_CONFIG.generationLockSeconds * 1000).toISOString();
  await admin.from("conversations").update({ generation_started_at: null }).eq("id", conversation.id).lt("generation_started_at", staleAt);
  const startedAt = new Date().toISOString();
  const { data: locked } = await admin.from("conversations").update({ generation_started_at: startedAt }).eq("id", conversation.id).is("generation_started_at", null).select("id").maybeSingle();
  ...
  if ((count ?? 0) >= AI_CONFIG.rateMaxMessages) { await release(); return new Response("Muitas mensagens em pouco tempo. Aguarde alguns minutos.", { status: 429 }); }

  const { error: insertError } = await supabase.from("messages").insert({ organization_id: patient.organization_id, conversation_id: conversation.id, sender_type: "patient", sender_user_id: user.id, content, client_message_id: body.clientMessageId, metadata: { source: "patient_portal" } });
  if (insertError) { await release(); return new Response(insertError.code === "23505" ? "Mensagem já recebida." : "Não foi possível salvar a mensagem.", { status: insertError.code === "23505" ? 409 : 500 }); }
  ...
  try {
    ...
    return new Response(new ReadableStream({ async start(controller) { try { for await (const event of stream) { if (event.type === "response.output_text.delta") { fullText += event.delta; controller.enqueue(encoder.encode(event.delta)); } else if (event.type === "response.completed") { responseId = event.response.id; usage = event.response.usage ? { input_tokens: event.response.usage.input_tokens, output_tokens: event.response.usage.output_tokens, total_tokens: event.response.usage.total_tokens } : null; } } if (!fullText.trim()) throw new Error("Empty AI response"); await admin.from("messages").insert({ organization_id: patient.organization_id, conversation_id: conversation.id, sender_type: "ai", content: fullText, metadata: { provider: "openai", model: AI_CONFIG.model, response_id: responseId, latency_ms: Date.now() - requestStarted, prompt_version: PATIENT_ASSISTANT_PROMPT_VERSION, usage } }); await admin.from("conversations").update({ generation_started_at: null, last_message_at: new Date().toISOString() }).eq("id", conversation.id); controller.close(); } catch (error) { await release(); console.error("patient_chat_generation_failed", { conversationId: conversation.id, error: error instanceof Error ? error.message : "unknown" }); controller.error(error); } } }), { headers: { "content-type": "text/plain; charset=utf-8", "cache-control": "no-store", "x-content-type-options": "nosniff" } });
